...
export const HeroBackgroundWorkspace: React.FC<HeroBackgroundWorkspaceProps> = ({
  ...
  const [copiedTailwind, setCopiedTailwind] = useState(false);
   ...
   const [copiedComponentCode, setCopiedComponentCode] = useState(false);
  ...
  const generateComponentCode = useCallback((): string => {
    ...
    const glowShadow = gradient === "glow" ? `\n      boxShadow: "0 0 20px ${gradientColor}60, 0 0 40px ${gradientColor}30",` : "";

    return `import React from "react";

/**
 * Custom Button Component
 * Generated by Beymflow Color Codes
 */
...
export const Button: React.FC<ButtonProps> = ({ children, onClick, className }) => {
  return (
    <button
      onClick={onClick}
      className={\`font-medium transition-all hover:opacity-90 hover:scale-[1.02] active:scale-[0.98] \${className || ""}\`}
      style={{
        background: ${getBackground()},
        color: "${text}",
        borderRadius: "${radius}px",
        padding: "${py}px ${px}px",${glowShadow}
      }}
    >
      {children}
    </button>
...
`;
  ...
  const handleCopyComponentCode = useCallback(async () => {
    const success = await robustCopyToClipboard(generateComponentCode());
    if (success) {
      setCopiedComponentCode(true);
      toast.success("Component code copied!");
      setTimeout(() => setCopiedComponentCode(false), 2500);
    }
