...
export function GlowingEffectDemo() {
  return <section className="grid gap-8 md:grid-cols-12 md:items-stretch">
      ...
      <ul className="md:col-span-7 grid grid-cols-1 gap-4 sm:grid-cols-2">
        <GridItem icon={<Box className="h-4 w-4 text-cyan-300" />} title="Effortless" description="Beymflow makes creation feel light. Build ideas, brands, and content faster than ever — without the technical weight slowing you down." />
        <GridItem icon={<Zap className="h-4 w-4 text-cyan-300" />} title="Fast" description="Every tool and prompt is engineered for speed. More output, less friction. Speed becomes your new creative advantage." />
        <GridItem icon={<Layers className="h-4 w-4 text-purple-300" />} title="Scalable" description="Whether you’re starting out or already seasoned, Beymflow grows with you. Your tools expand as your vision expands." />
        <GridItem icon={<Target className="h-4 w-4 text-purple-300" />} title="Precise" description="Every command, every workflow, every idea is refined for clarity. You get consistent, reliable results — every single time." />
      </ul>
    </section>;
}
