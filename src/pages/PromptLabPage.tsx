...
const CustomSelect = ({ label, icon: Icon, value, options, onChange }: { label: string, icon: any, value: string, options: string[], onChange: (val: string) => void }) => {
  ...
  return (
    <div className="space-y-1 relative" ref={containerRef}>
      ...
      <button onClick={() => setIsOpen(!isOpen)} className="w-full bg-black/40 backdrop-blur-md border border-white/10 rounded-xl px-3 py-2 text-xs text-white flex items-center justify-between hover:bg-white/5 transition-colors focus:border-white/30 outline-none"><span>{value}</span><ChevronDown size={12} className={`transition-transform ${isOpen ? "rotate-180" : ""}`} /></button>
      <AnimatePresence>
        {isOpen && (
          <motion.div initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 5 }} className="absolute top-full left-0 right-0 mt-1 bg-[#161618]/95 backdrop-blur-xl border border-white/10 rounded-xl overflow-hidden z-50 shadow-2xl">
            <div className="max-h-40 overflow-y-auto custom-scrollbar p-1">
                {options.map((opt) => (<button key={opt} onClick={() => { onChange(opt); setIsOpen(false); }} className={`w-full text-left px-3 py-2 text-xs rounded-lg transition-colors ${value === opt ? "text-white bg-white/10 font-bold" : "text-neutral-400 hover:bg-white/5 hover:text-white"}`}>{opt}</button>))}
            </div>
          </motion.div>
        )}
      </AnimatePresence>
...
*(Self-Correction mechanism: Before outputting, verify that the advice is actionable and specific to the context of "${base}".)*`;
  ...
  return (
    <div className="relative h-screen w-full bg-black text-white flex flex-col font-sans overflow-hidden selection:bg-cyan-500/30 selection:text-cyan-100">
      <div className="fixed inset-0 z-0 opacity-30 pointer-events-none" style={{ backgroundImage: 'url("https://raw.githubusercontent.com/multiply-dev/multiply-dark-web-forge/main/public/lovable-uploads/IMG_0468.JPEG")', backgroundSize: "cover", backgroundPosition: "center" }} />
      ...
      <AnimatePresence mode="wait">
        {viewMode === "landing" ? (
          <motion.main key="landing" exit={{ opacity: 0, filter: "blur(10px)" }} className="flex-1 flex flex-col items-center p-4 w-full max-w-7xl mx-auto z-10 relative h-full overflow-y-auto custom-scrollbar [&_input]:caret-transparent [&_input:focus]:caret-white [&_textarea]:caret-transparent [&_textarea:focus]:caret-white">
              <button onClick={() => navigate("/")} className="absolute top-4 left-4 z-50 p-2 rounded-full bg-white/5 border border-white/10 text-neutral-400 hover:text-white hover:bg-white/10 flex items-center gap-2"><Home size={20} /><span className="text-sm font-medium">Home</span></button>
              ...
              <div className="w-full max-w-4xl mx-auto mb-32 px-4 relative z-20">
                <form onSubmit={(e) => { e.preventDefault(); if(landingInput.trim()) initializeWorkspace("business", landingInput, true); }} className="relative group flex flex-col items-center justify-center" onClick={() => document.getElementById('main-input')?.focus()}>
                  <div className="relative flex flex-wrap justify-center items-center text-2xl md:text-4xl font-light tracking-tight cursor-text min-h-[60px] w-full text-center">
                      <input id="main-input" type="text" value={landingInput} onChange={handleInputChange} onSelect={handleInputSelect} onFocus={() => setIsFocused(true)} onBlur={() => setIsFocused(false)} className="absolute inset-0 w-full h-full opacity-0 z-10 cursor-text caret-transparent text-center" autoFocus autoComplete="off" />
                ...
                <div className="max-w-6xl mx-auto flex flex-col gap-8">
                  ...
                  <div className="flex flex-col gap-6 border-b border-white/10 pb-8">
                    ...
                    <div className="flex items-center justify-center gap-4 w-full pt-4 border-t border-white/5">
                      <div className="relative w-full max-w-md"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-neutral-500" size={18} /><input type="text" placeholder={selectedTier === 'premium' ? "Search premium..." : "Search free..."} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-[#1E1E20] border border-white/10 rounded-xl py-3 pl-10 pr-4 text-sm text-white focus:outline-none focus:border-purple-500/50 transition-colors placeholder:text-neutral-600" /></div>
