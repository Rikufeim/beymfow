...
const WIDGET_CATALOG = [
  {
    type: "input",
    title: "User Protocol",
    desc: "Primary input interface for defining goals.",
    icon: FileText,
    color: "text-cyan-400",
  },
  {
    type: "engine",
    title: "Neural Engine",
    desc: "Visualizes the AI processing steps.",
    icon: Cpu,
  ...
  },
  {
    ...
    desc: "Displays the optimized prompt output.",
    icon: Wand2,
    color: "text-purple-400",
  },
  {
    ...
    desc: "Test prompts against GPT-4, Claude & Gemini.",
    icon: Box,
    color: "text-green-400",
  },
];
...
function PromptWorkspaceInner() {
  ...
  useEffect(() => {
    if (viewMode !== "landing" || landingInput.length > 0 || isFocused) return;
    const interval = setInterval(() => {
      setPlaceholderIndex((prev) => (prev + 1) % PLACEHOLDERS.length);
    }, 4000);
    return () => clearInterval(interval);
  }, [viewMode, landingInput, isFocused]);
  ...
  const handleFormSubmit = useCallback(
    (e: React.FormEvent) => {
      e.preventDefault();
      if (landingInput.trim()) initializeWorkspace("business", landingInput, true);
    },
    [landingInput, initializeWorkspace],
  );
  ...
  useEffect(() => {
    ...
    const addLog = (text: string) =>
      setAgentLogs((prev) => [
        ...prev,
        { id: Date.now(), text, status: "done", timestamp: new Date().toLocaleTimeString() },
      ]);
    ...
    const runSimulation = async () => {
      ...
      addLog("🧠 Applying advanced prompt engineering...");
      setProgress(70);
      await new Promise((r) => setTimeout(r, 600));
      addLog("✨ Finalizing output structure...");
      setProgress(100);
  ...
  }, [selectedGoal, runId, generateFinalPrompt]);
  ...
  const generateFinalPrompt = useCallback(() => {
    ...
    if (role === "Expert") {
      ...
      else if (txt.includes("code") || txt.includes("react") || txt.includes("python"))
        calculatedRole = "Senior Principal Engineer";
...
*(Self-Correction mechanism: Before outputting, verify that the advice is actionable and specific to the context of "${base}".)*`;
  ...
  }, [selectedGoal, userText, refinementSettings]);

  // --- MULTI-MODEL TEST LOGIC ---
  const handleRunMultiModelTest = useCallback(
    (widgetId: string) => {
      setWidgets((prev) =>
        prev.map((w) => {
  ...
  const handleToggleModel = useCallback((widgetId: string, modelId: string) => {
    setWidgets((prev) =>
      prev.map((w) => {
        if (w.id !== widgetId) return w;
  ...
  const handleWidgetPromptChange = useCallback((widgetId: string, text: string) => {
    setWidgets((prev) => prev.map((w) => (w.id === widgetId ? { ...w, data: { ...w.data, prompt: text } } : w)));
  }, []);

  // Refinement settings handlers
  const handleRoleChange = useCallback((val: string) => setRefinementSettings((prev) => ({ ...prev, role: val })), []);
  const handleToneChange = useCallback((val: string) => setRefinementSettings((prev) => ({ ...prev, tone: val })), []);
  const handleFormatChange = useCallback(
    (val: string) => setRefinementSettings((prev) => ({ ...prev, format: val })),
    [],
  );
  const handleComplexityChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) =>
      setRefinementSettings((prev) => ({ ...prev, complexity: parseInt(e.target.value) })),
    [],
  );
  ...
  // --- WORKSPACE INIT ---
  const initializeWorkspace = useCallback((selectedDomain: Domain, initialText?: string, autoRun: boolean = false) => {
  ...
  const handleAddWidget = useCallback(
    (widgetDef: (typeof WIDGET_CATALOG)[0]) => {
  ...
  const handleMouseDown = useCallback(
    (e: React.MouseEvent, type: "widget" | "pinned", id: string, index?: number) => {
    ...
    const handleMouseUp = (e: MouseEvent) => {
      if (dragging) {
        if (dragging.type === "pinned" && dropTargetId && dragging.index !== undefined) {
          ...
          setWidgets((prev) =>
            prev.map((w) => (w.id === dropTargetId ? { ...w, data: { ...w.data, prompt: promptText } } : w)),
          );
  ...
  }, [dragging, pinnedPrompts, widgets, dropTargetId]);

  // Separate resize effect with throttling
        ...
        setWidgets((prev) =>
          prev.map((w) =>
            w.id === resizing.id
              ? {
                  ...w,
                  width: Math.max(300, resizing.startWidth + (e.clientX - resizing.startX)),
                  height: Math.max(200, resizing.startHeight + (e.clientY - resizing.startY)),
                }
              : w,
          ),
        );
      ...
      <AnimatePresence mode="wait">
        {viewMode === "landing" ? (
                ...
                <div className="relative flex flex-wrap justify-center items-center text-2xl md:text-4xl font-light tracking-tight cursor-text min-h-[60px] w-full text-center">
                  ...
                  {landingInput.length === 0 && !isFocused ? (
                    ...
                    <div className="relative z-0 flex items-center justify-center break-all">
                      <span className="text-white whitespace-pre-wrap relative">
                        {landingInput.slice(0, cursorPos)}
                      </span>
                      <motion.div
                        animate={{
                          opacity: [1, 0.5, 1],
                            ...
                            {(activePrompt.tier === "free" || isPremiumUnlocked) && (
                              <div className="flex items-center gap-3 mt-4">
                                ...
                                <button
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    initializeWorkspace(activePrompt.domain, activePrompt.prompt, true);
                                  }}
                                ...
                                >
                                  ...
                                  <span>Open in Workspace & Run</span>
                                </button>
                              </div>
                            )}
                          </div>
          ...
          <motion.div
            ...
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="flex-1 relative w-full h-full flex flex-col z-10"
          >
            {/* Grid Overlay */}
            <div
            ...
            <div className="absolute top-4 right-4 z-30 flex items-center gap-3">
              <button
                ...
                className={`h-10 px-4 rounded-lg backdrop-blur-md border flex items-center gap-2 shadow-lg transition-all ${isLibraryOpen ? "bg-white text-black border-white" : "bg-white/5 border-white/10 text-neutral-300 hover:text-white"}`}
              >
                <PanelRight size={18} />
            ...
            <div ref={canvasRef} className="flex-1 relative overflow-auto z-0 custom-scrollbar">
                  ...
                  >
                    ...
                    <div className="px-4 py-3 border-b border-white/5 bg-white/[0.02] flex items-center justify-between cursor-move select-none">
                      ...
                      <button
                        ...
                        className="text-neutral-600 hover:text-red-400 transition-colors no-drag"
                      >
                        <Trash2 size={14} />
                      </button>
                    </div>
                    ...
                    <div className="flex-1 bg-[#161618] overflow-hidden flex flex-col relative">
                      {widget.type === "multi-model" && widget.data && (
                        <div className="flex flex-col h-full p-4 gap-4 scroll-container overflow-y-auto custom-scrollbar">
                          ...
                          <div className="flex flex-col gap-2">
                            ...
                            <textarea
                              value={widget.data.prompt}
                              onChange={(e) => handleWidgetPromptChange(widget.id, e.target.value)}
                              placeholder="Drag & drop a prompt here or type..."
                              className={`w-full h-32 bg-black/30 border rounded-lg p-3 text-sm text-white placeholder:text-neutral-600 focus:outline-none focus:border-white/30 resize-none font-mono transition-colors ${isDropTarget ? "border-purple-500/50 bg-purple-500/10" : "border-white/10"}`}
                            />
                          </div>
                          <div className="flex flex-col gap-2">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                              {AVAILABLE_MODELS.map((model) => {
                                ...
                                return (
                                  <button
                                    key={model.id}
                                    onClick={() => handleToggleModel(widget.id, model.id)}
                                    className={`flex items-center gap-2 p-2 rounded-lg border text-xs transition-all ${isSelected ? `bg-white/10 border-white/30 text-white` : "bg-transparent border-white/5 text-neutral-500 hover:bg-white/5"}`}
                                  >
                                    <div
                                      className={`w-3 h-3 rounded-full ${isSelected ? "bg-green-500" : "bg-neutral-700"}`}
                                    />
                          ...
                          <button
                            onClick={() => handleRunMultiModelTest(widget.id)}
                            disabled={
                              widget.data.isTesting || !widget.data.prompt || widget.data.selectedModels.length === 0
                            }
                            className="w-full py-3 bg-white text-black rounded-lg text-xs font-bold uppercase hover:bg-neutral-200 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                          >
                            {widget.data.isTesting ? (
                              <Loader2 size={14} className="animate-spin" />
                            ) : (
                              <Play size={14} fill="black" />
                            )}
                            {widget.data.isTesting ? "Testing..." : "Test Prompt"}
                          </button>
                          <div className="grid grid-cols-1 gap-4 mt-2 pb-4">
                            {widget.data.results.map((res: any) => {
                              ...
                              return (
                                <div
                                  key={res.model}
                                  className="bg-[#0F0F10] border border-white/10 rounded-lg overflow-hidden"
                                >
                                  <div className="px-3 py-2 bg-white/5 border-b border-white/5 flex items-center justify-between">
                                    <div className="flex items-center gap-2">
                                      <modelInfo.icon size={12} className={modelInfo?.color} />
                                      <span className="text-xs font-bold text-neutral-300">{modelInfo?.name}</span>
                                    </div>
                                    {res.status === "loading" && (
                                      <Loader2 size={12} className="animate-spin text-neutral-500" />
                                    )}
                                    {res.status === "success" && <CheckCircle2 size={12} className="text-green-500" />}
                                    {res.status === "error" && <AlertCircle size={12} className="text-red-500" />}
                                  </div>
                                  <div className="p-3 text-xs text-neutral-300 font-mono leading-relaxed min-h-[60px]">
                                    {res.status === "loading" ? (
                                      <span className="text-neutral-600 animate-pulse">Fetching response...</span>
                                    ) : (
                                      res.response || <span className="text-neutral-700 italic">Waiting to run</span>
                                    )}
                                  </div>
                                </div>
                              );
                      ...
                      {widget.type === "input" && (
                        <div className="flex flex-col h-full p-4 gap-4 scroll-container overflow-y-auto custom-scrollbar">
                          <textarea
                            ...
                            className="w-full bg-[#131314] border border-white/10 rounded-lg p-4 text-sm text-white placeholder:text-neutral-600 focus:outline-none focus:border-white/30 resize-none min-h-[120px] font-sans"
                          ...
                          <div className="flex items-center justify-between border-b border-white/5 pb-2">
                            ...
                            <button
                              onClick={() => setGoalVariation((p) => p + 1)}
                          ...
                          <div className="space-y-2 flex-1">
                            {userText.trim().length === 0 ? (
                              <div className="h-24 flex items-center justify-center text-neutral-700 text-xs italic border border-dashed border-white/5 rounded-lg">
                                Input text required
                              </div>
                            ) : (
                              goalOptions.map((g, i) => (
                                <button
                                  ...
                                  className={`w-full text-left p-3 rounded-lg border text-xs transition-all ${selectedGoal === g ? `bg-white/5 border-${DOMAINS[0].color.replace("text-", "border-")}` : "border-white/5 text-neutral-400 hover:bg-white/5 hover:text-white"}`}
                                >
                                  {g}
                                </button>
                              ))
                      ...
                      {widget.type === "engine" && (
                        <div className="flex flex-col h-full p-4 scroll-container overflow-y-auto custom-scrollbar bg-[#0a0a0a]">
                            ...
                            <div className="space-y-3 font-mono text-xs">
                              {agentLogs.map((log) => (
                                ...
                                >
                                  ...
                                  <span className={log.text.includes("✓") ? "text-green-400" : "text-neutral-300"}>
                                    {log.text}
                                  </span>
                                </motion.div>
                              ))}
                              {stage === "agent" && (
                                <div className="text-amber-500 animate-pulse mt-4 pl-16">PROCESSING_THREAD...</div>
                              )}
                            </div>
                          ) : (
                          ...
                          {stage !== "idle" && (
                            <div className="absolute bottom-0 left-0 right-0 h-1 bg-[#1E1E20]">
                        ...
                        <div className="flex flex-col h-full p-0 scroll-container overflow-hidden">
                          <div className="flex-1 p-4 overflow-y-auto custom-scrollbar bg-[#0F0F10]">
                            {resultPrompt ? (
                              <textarea
                                readOnly
                                className="w-full h-full bg-transparent text-neutral-200 font-mono text-sm leading-relaxed resize-none focus:outline-none"
                                value={resultPrompt}
                              />
                            ) : (
                              <div className="h-full flex items-center justify-center flex-col gap-4 text-neutral-600">
                                <Wand2 size={32} className="opacity-20" />
                                <span className="text-xs uppercase tracking-widest">Ready to Optimize</span>
                              </div>
                            )}
                          </div>
                          <div
                            className="border-t border-white/10 bg-black/40 backdrop-blur-md p-4 flex flex-col gap-4 no-drag"
                            onMouseDown={(e) => e.stopPropagation()}
                          >
                            <div className="grid grid-cols-2 gap-3">
                              <CustomSelect
                                label="Role"
                                icon={User}
                                value={refinementSettings.role}
                                options={["Expert", "Critic", "Teacher", "Consultant", "Visionary"]}
                                onChange={handleRoleChange}
                              />
                              <CustomSelect
                                label="Tone"
                                icon={MessageSquare}
                                value={refinementSettings.tone}
                                options={["Professional", "Creative", "Direct", "Empathetic", "Witty"]}
                                onChange={handleToneChange}
                              />
                              <CustomSelect
                                label="Format"
                                icon={FileType}
                                value={refinementSettings.format}
                                options={["Step-by-Step", "Essay", "Code Block", "Bullet Points", "Email"]}
                              ...
                              <div className="space-y-1">
                                ...
                                <input
                                  ...
                                  min="0"
                                  max="100"
                                  value={refinementSettings.complexity}
                            ...
                            <div className="flex gap-3 pt-2 border-t border-white/5">
                              <button
                                ...
                                className="flex-1 py-2 rounded-xl text-xs font-bold uppercase text-neutral-400 hover:bg-white/5 hover:text-white border border-transparent hover:border-white/10 disabled:opacity-30 flex items-center justify-center gap-2"
                              >
                                <Copy size={14} /> Copy
                              </button>
                              <button
                                ...
                                className="flex-[2] py-2 bg-purple-500/20 border border-purple-500/30 rounded-xl text-xs font-bold uppercase text-purple-300 hover:bg-purple-500/30 hover:text-white transition-all disabled:opacity-30 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                              >
                                <SlidersHorizontal size={14} /> Update Prompt
                              </button>
                            </div>
                          </div>
                        </div>
              ...
              {pinnedPrompts.map((pinned, i) => (
                <div
                  key={i}
                  className="absolute z-20 w-80 bg-[#0a0a0a] border border-white/20 rounded-xl shadow-2xl flex flex-col overflow-hidden backdrop-blur-md"
                  style={{ left: pinned.x, top: pinned.y, boxShadow: "0 20px 40px rgba(0,0,0,0.6)" }}
                  onMouseDown={(e) => handleMouseDown(e, "pinned", pinned.item.id, i)}
                >
                  <div className="px-3 py-2 bg-white/10 flex items-center justify-between cursor-move border-b border-white/10">
                    <span className="text-xs font-bold text-white flex items-center gap-2">
                      <GripHorizontal size={14} className="text-neutral-500" /> {pinned.item.title}
                    </span>
                    <button
                      onClick={() => setPinnedPrompts((p) => p.filter((_, idx) => idx !== i))}
                      className="text-neutral-400 hover:text-white"
                    >
                      <X size={14} />
                    </button>
                  </div>
                  <div className="p-3 text-xs text-neutral-300 font-mono max-h-48 overflow-y-auto custom-scrollbar leading-relaxed pointer-events-none">
                    {pinned.item.prompt}
                  </div>
                  <div className="p-2 border-t border-white/10 bg-black/40 flex justify-center text-[10px] text-neutral-500 font-bold uppercase">
                    Drag & Drop to Widget
                  </div>
                </div>
              ))}
            ...
            <AnimatePresence>
              {isLibraryOpen && (
                <motion.div
                  ...
                  className="absolute top-0 right-0 bottom-0 w-[400px] z-40 bg-[#0F0F10]/95 backdrop-blur-xl border-l border-white/10 flex flex-col shadow-2xl"
                >
                  <div className="p-6 border-b border-white/10 flex items-center justify-between">
                    <h2 className="text-xl font-bold text-white flex items-center gap-2">
                      <PanelRight size={20} className="text-neutral-400" /> Library
                    </h2>
                    <button
                      onClick={() => setIsLibraryOpen(false)}
                      className="p-2 bg-white/5 rounded-full text-neutral-400 hover:text-white"
                    >
                      <X size={16} />
                    </button>
                  </div>
                  <div className="flex border-b border-white/10">
                    <button
                      ...
                      className={`flex-1 py-3 text-sm font-bold uppercase tracking-wide transition-colors ${libraryTab === "prompts" ? "text-white border-b-2 border-white bg-white/5" : "text-neutral-500 hover:text-neutral-300"}`}
                    >
                      Prompts
                    </button>
                    <button
                      onClick={() => setLibraryTab("widgets")}
                      className={`flex-1 py-3 text-sm font-bold uppercase tracking-wide transition-colors ${libraryTab === "widgets" ? "text-white border-b-2 border-white bg-white/5" : "text-neutral-500 hover:text-neutral-300"}`}
                    >
                      Widgets
                    </button>
                  </div>
                      ...
                      <div className="p-4 border-b border-white/5">
                        <div className="relative">
                          ...
                          <input
                            ...
                            value={searchQuery}
                            onChange={(e) => setSearchQuery(e.target.value)}
                            className="w-full bg-white/5 backdrop-blur-md border border-white/10 rounded-lg py-2 pl-9 pr-4 text-xs text-white focus:outline-none focus:border-white/30"
                          />
                        </div>
                      </div>
                        ...
                        {filteredPrompts.map((prompt) => (
                          <div
                            ...
                            onClick={() =>
                              setPinnedPrompts((p) => [
                                ...p,
                                { item: prompt, x: 100 + p.length * 20, y: 100 + p.length * 20 },
                              ])
                            }
                            className="group p-4 rounded-xl border border-white/10 bg-white/5 hover:bg-white/10 transition-all cursor-pointer hover:scale-[1.02] active:scale-[0.98]"
                          >
                            <div className="flex justify-between items-start mb-2">
                              <div className="flex items-center gap-2">
                                <span className="text-[10px] font-bold uppercase px-1.5 py-0.5 rounded bg-white/10 text-neutral-400">
                                  {prompt.tier}
                                </span>
                                <span className="text-[10px] uppercase font-bold text-cyan-400">{prompt.domain}</span>
                              </div>
                              <MousePointer2 size={12} className="opacity-0 group-hover:opacity-50" />
                            </div>
                        ...
                        return (
                          ...
                          >
                            <div className="flex items-center justify-between mb-2">
                              <div className="flex items-center gap-3">
                                <div className={`p-2 rounded-lg bg-white/5 ${w.color}`}>
                                  <w.icon size={20} />
                                </div>
                                <h4 className="font-bold text-sm text-white">{w.title}</h4>
                              </div>
                              {isActive && w.type !== "multi-model" && (
                                <span className="text-[10px] uppercase font-bold text-green-400 bg-green-900/20 px-2 py-1 rounded">
                                  Active
                                </span>
                              )}
                            </div>
