...
const CustomSelect = ({ label, icon: Icon, value, options, onChange }: { label: string, icon: any, value: string, options: string[], onChange: (val: string) => void }) => {
  ...
  return (
    <div className="space-y-1 relative" ref={containerRef}>
      ...
      <button onClick={() => setIsOpen(!isOpen)} className="w-full bg-white/5 backdrop-blur-md border border-white/10 rounded-xl px-3 py-2 text-xs text-white flex items-center justify-between hover:bg-white/10 transition-colors focus:border-white/30 outline-none"><span>{value}</span><ChevronDown size={12} className={`transition-transform ${isOpen ? "rotate-180" : ""}`} /></button>
      <AnimatePresence>
        {isOpen && (
      ...
      <AnimatePresence mode="wait">
        {viewMode === "landing" ? (
          <motion.main key="landing" exit={{ opacity: 0, filter: "blur(10px)" }} className="flex-1 flex flex-col items-center p-4 w-full max-w-7xl mx-auto z-10 relative h-full overflow-y-auto custom-scrollbar [&_input]:caret-transparent [&_input:focus]:caret-white [&_textarea]:caret-transparent [&_textarea:focus]:caret-white">
              <button onClick={() => navigate("/")} className="absolute top-4 left-4 z-50 p-2 rounded-full bg-white/5 backdrop-blur-md border border-white/10 text-neutral-400 hover:text-white hover:bg-white/10 transition-all flex items-center gap-2"><Home size={20} /><span className="text-sm font-medium">Home</span></button>
              <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full flex flex-col items-center justify-center min-h-[60vh] text-center py-20 px-4">
                ...
                <div className="inline-block bg-white/5 backdrop-blur-md rounded-2xl px-8 py-3 mb-12 border border-white/10"><p className="text-lg text-white/90 font-medium text-center">By Beymflow</p></div>
              </motion.section>
              ...
              <div className="w-full max-w-4xl mx-auto mb-32 px-4 relative z-20">
                <form onSubmit={(e) => { e.preventDefault(); if(landingInput.trim()) initializeWorkspace("business", landingInput, true); }} className="relative group flex flex-col items-center justify-center" onClick={() => document.getElementById('main-input')?.focus()}>
                  ...
                  <div className="h-12 mt-4 flex items-center justify-center"><AnimatePresence>{landingInput.trim().length > 0 && (<motion.button initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} type="submit" className="group flex items-center gap-2 px-6 py-2 bg-white/90 backdrop-blur-md text-black rounded-full font-bold text-sm hover:bg-white transition-all shadow-lg shadow-purple-500/20 cursor-pointer z-20"><span>Enter the Lab</span><ArrowRight size={16} className="group-hover:translate-x-1 transition-transform" /></motion.button>)}</AnimatePresence></div>
                </form>
              </div>
          ...
          <motion.div key="workspace" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex-1 relative w-full h-full flex flex-col z-10">
            ...
            {/* Nav */}
            <div className="absolute top-4 left-4 z-30"><button onClick={() => setViewMode("landing")} className="h-10 px-4 rounded-lg bg-white/5 backdrop-blur-md border border-white/10 text-neutral-300 hover:bg-white/10 hover:text-white flex items-center gap-2 transition-all shadow-lg cursor-pointer font-sans"><ArrowLeft size={18} /><span className="text-sm font-medium">Back</span></button></div>
            <div className="absolute top-4 right-4 z-30 flex items-center gap-3"><button onClick={() => setIsLibraryOpen(!isLibraryOpen)} className={`h-10 px-4 rounded-lg backdrop-blur-md border flex items-center gap-2 shadow-lg transition-all ${isLibraryOpen ? "bg-white text-black border-white" : "bg-white/5 border-white/10 text-neutral-300 hover:text-white"}`}><PanelRight size={18} /><span className="text-sm font-bold">Library</span></button></div>
            ...
            <AnimatePresence>
              {isLibraryOpen && (
                <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "spring", stiffness: 300, damping: 30 }} className="absolute top-0 right-0 bottom-0 w-[400px] z-40 bg-[#0F0F10]/95 backdrop-blur-xl border-l border-white/10 flex flex-col shadow-2xl">
                    ...
                    {libraryTab === "prompts" && (
                       <>
                        <div className="p-4 border-b border-white/5"><div className="relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-neutral-500" size={14} /><input type="text" placeholder="Search..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-white/5 backdrop-blur-md border border-white/10 rounded-lg py-2 pl-9 pr-4 text-xs text-white focus:outline-none focus:border-white/30" /></div></div>
