...
// Removed useNavigate import as it caused errors without a Router context
        ...
        {viewMode === "landing" ? (
          ...
          >
            <button
              ...
              className="fixed top-4 left-4 z-50 p-2 rounded-full bg-white/5 backdrop-blur-md border border-white/10 text-neutral-400 hover:text-white hover:bg-white/10 transition-all flex items-center gap-2"
            >
              <Home size={20} />
                ...
                <div className="flex flex-col gap-6 border-b border-white/10 pb-8">
                  ...
                  <AnimatePresence>
                    {filterDomain !== "all" && (
                      ...
                      >
                        {DOMAINS.find((d) => d.key === filterDomain)?.subcategories.map((sub) => (
                          <button
                            key={sub}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 min-h-[400px]">
                  {filteredPrompts.length > 0 ? (
                    filteredPrompts.map((prompt) => {
                      const domainConfig = DOMAINS.find((d) => d.key === prompt.domain);
                      const isPremium = prompt.tier === "premium";
                      return (
                        // OPTIMIZED CARD RENDER: Removed layoutId to prevent morphing animation
                        <motion.div
                          key={prompt.id}
                          ...
                          <div className="relative z-10">
                            ...
                            <div className="bg-white/5 backdrop-blur-md rounded-xl p-4 mb-4 border border-white/5 relative overflow-hidden group-hover:border-white/10 transition-colors h-24">
                              {isPremium && !isPremiumUnlocked ? (
                                ...
                                <p className="text-sm text-neutral-300 font-mono line-clamp-3 opacity-70">
                                  {prompt.prompt}
                                </p>
                              )}
                            </div>
            ...
            <AnimatePresence>
              {activePrompt && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center px-4">
                  ...
                  />
                  {/* MODAL: Removed layoutId to prevent morphing, added simple fade/scale */}
                  <motion.div
                  ...
                  >
                    ...
                    <div className="flex-1 overflow-y-auto custom-scrollbar">
                      <div className="flex items-center gap-2 mb-4">
                        <span
                          className={`text-xs font-bold uppercase tracking-wider ${DOMAINS.find((d) => d.key === activePrompt.domain)?.color}`}
                        >
                          {DOMAINS.find((d) => d.key === activePrompt.domain)?.label}
                        </span>
                          ...
                          >
                            <div className="grid grid-cols-2 gap-3">
                              <CustomSelect
                                ...
                                onChange={(val) => setRefinementSettings({ ...refinementSettings, role: val })}
                              />
                              <CustomSelect
                                label="Tone"
                                icon={MessageSquare}
                                ...
                                onChange={(val) => setRefinementSettings({ ...refinementSettings, tone: val })}
                              />
                              <CustomSelect
                                label="Format"
                                icon={FileType}
                                ...
                                onChange={(val) => setRefinementSettings({ ...refinementSettings, format: val })}
                              />
                              <div className="space-y-1">
                                <label className="text-[10px] uppercase font-bold text-neutral-500 flex items-center gap-1">
