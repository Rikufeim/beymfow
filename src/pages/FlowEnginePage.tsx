...
interface Widget {
  ...
  width: number;
  height: number;
  promptMode?: "edit" | "preview"; // For prompt-window: edit = Prompt Mode, preview = Preview Mode
}
...
const FlowEngineContent: React.FC<FlowEngineProps> = ({ onBack }) => {
  ...
  const handleGeneratePrompt = () => {
    const prompt = generatePromptFromGraph();
  ...
  // Update Prompt Window preview content when nodes change
  useEffect(() => {
    const promptWindows = widgets.filter((w) => w.type === "prompt-window" && w.promptMode === "preview");
    if (promptWindows.length === 0) return;

    const updatedContent = buildPromptFromNodes();
    setWidgets((prev) =>
      prev.map((w) => {
        if (w.type === "prompt-window" && w.promptMode === "preview") {
    ...
    widgets.filter((w) => w.type === "category" || w.type.startsWith("flow-")).length,
    JSON.stringify(
      widgets
        .filter((w) => w.type === "category" || w.type.startsWith("flow-"))
        .map((w) => ({ id: w.id, content: w.content })),
    ),
    JSON.stringify(nodeOutputMap),
    mainPromptState.combinedPrompt,
  ]);
  ...
  // Build prompt content from all nodes in workspace (legacy function for prompt-window)
  const buildPromptFromNodes = (): string => {
    ...
    if (categoryNodes.length > 0) {
      promptContent += "## Category Specifications\n\n";
      categoryNodes.forEach((node) => {
        if (node.category && node.content?.trim()) {
          promptContent += `### ${node.category.name}\n${node.content.trim()}\n\n`;
        }
      });
    }
    ...
    if (flowNodes.length > 0) {
      promptContent += "## Flow Node Outputs\n\n";
      flowNodes.forEach((node) => {
        const nodeOutput = nodeOutputMap[node.id];
        const content = nodeOutput?.generatedText || node.content || "";
        if (content.trim()) {
          const title = node.title || node.subtitle || "Untitled Node";
          promptContent += `### ${title}\n${content.trim()}\n\n`;
        }
      });
    }
  ...
  // Generate prompt from graph following topological order based on connections
  const generatePromptFromGraph = (): string => {
    if (widgets.length === 0) {
      return "No nodes in workspace. Add nodes to build your prompt.";
    }
    ...
    // Find root nodes (nodes with no incoming edges) or all nodes if no edges
    const allNodeIds = new Set(widgets.map((w) => w.id));
    const nodesWithIncoming = new Set(incomingEdges.keys());
    const rootNodes = widgets.filter((w) => !nodesWithIncoming.has(w.id));

    // If we have edges, use topological sort. Otherwise, just process all nodes.
    ...
    // Start from root nodes or all nodes if no edges
    if (rootNodes.length > 0) {
      rootNodes.forEach((root) => visit(root.id));
    } else if (edges.length === 0) {
      // No edges, process all nodes in their current order
      widgets.forEach((w) => result.push(w));
    } else {
      // Fallback: process all nodes
      widgets.forEach((w) => visit(w.id));
    }

    // Build prompt from nodes in topological order
    const sections: string[] = [];

    result.forEach((node) => {
      let content = "";
      let title = node.title || node.subtitle || "Untitled Node";
      ...
      if (node.type === "category") {
        if (node.category && node.content?.trim()) {
          ...
          // Skip category nodes with no content
          return;
        }
      } else if (node.type.startsWith("flow-")) {
        const nodeOutput = nodeOutputMap[node.id];
        // For flow-input nodes, always use content if available
        if (node.type === "flow-input") {
          content = node.content || "";
        } else {
          // For other flow nodes, prefer generatedText
          content = nodeOutput?.generatedText || node.content || "";
        }
        // Skip flow nodes with no content (unless it's an input node that might be empty)
        if (!content.trim() && node.type !== "flow-input") {
          return;
        }
      ...
      } else if (node.type === "prompt-window") {
        // Skip prompt-window nodes from generation
        return;
      } else {
        // Unknown node type, try to get content
        content = node.content || "";
        if (!content.trim()) {
          return;
        }
      ...
      if (content.trim()) {
        // Use node title/subtitle to determine section structure
        const sectionTitle = title.trim();

        // Format based on node type
        if (node.type === "flow-input" || node.type === "category") {
    ...
    // Use mainPromptState if available
    if (mainPromptState.combinedPrompt && sections.length === 0) {
      finalPrompt = mainPromptState.combinedPrompt;
    }

    // If no content found, show helpful message
    if (!finalPrompt.trim()) {
      finalPrompt = "No content available. Add nodes to the workspace and fill in their content to build your prompt.";
    }
  ...
  // Handler to add Prompt Window node
  const handleAddPromptWindow = () => {
    // Calculate position (center of viewport or offset from existing nodes)
    const viewportCenterX = canvasRef.current
      ? (window.innerWidth / 2 - canvasTransform.translateX) / canvasTransform.scale
      : 400;
    const viewportCenterY = canvasRef.current
      ? (window.innerHeight / 2 - canvasTransform.translateY) / canvasTransform.scale
      : 300;

    // Find a good position (avoid overlap with existing nodes)
    let adjustedX = viewportCenterX;
    let adjustedY = viewportCenterY;

    if (widgets.length > 0) {
      const existingBounds = {
        minX: Math.min(...widgets.map((w) => w.x)),
        maxX: Math.max(...widgets.map((w) => w.x + w.width)),
        minY: Math.min(...widgets.map((w) => w.y)),
        maxY: Math.max(...widgets.map((w) => w.y + w.height)),
      };
      adjustedX = existingBounds.maxX + 50;
      adjustedY = existingBounds.minY;
    }

    const initialPrompt = buildPromptFromNodes();
    ...
    const newWidget: Widget = {
      ...
      width: 500,
      height: 400,
      promptMode: "edit", // Start in edit mode
    };
            ...
            <div className="absolute top-0 left-0 right-0 h-14 bg-neutral-900 border-b border-neutral-800 flex items-center justify-between px-4 z-30">
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                <button
                  ...
                  className="h-10 px-4 rounded-lg border border-neutral-800 bg-neutral-900/80 text-neutral-300 hover:bg-neutral-800 hover:text-white hover:border-neutral-700 flex items-center gap-2 transition-all shadow-lg cursor-pointer backdrop-blur-md text-sm font-medium"
                  title="Generate prompt from graph"
                >
                  <Sparkles size={16} />
            ...
            <AnimatePresence>
              {showPromptPanel && (
                <motion.div
                  ...
                  className="absolute top-14 right-4 w-[480px] max-h-[calc(100vh-80px)] bg-[#121214] border border-neutral-800 rounded-lg z-40 flex flex-col shadow-2xl"
                >
                  {/* Panel Header */}
                  <div className="flex items-center justify-between p-4 border-b border-neutral-800">
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className="px-3 py-1.5 rounded-md bg-neutral-800 hover:bg-neutral-700 text-neutral-300 hover:text-white transition-colors flex items-center gap-2 text-xs font-medium"
                  ...
                  <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
                    <pre className="font-mono text-xs text-neutral-300 whitespace-pre-wrap break-words leading-relaxed">
                      {generatedPrompt || "No prompt generated yet. Click 'Generate Prompt' to create one."}
                    </pre>
                  </div>

                  {/* Panel Footer */}
                  <div className="p-3 border-t border-neutral-800 bg-neutral-900/50">
                    <p className="text-xs text-neutral-500">Ready to paste into Lovable, ChatGPT, or other AI tools</p>
                  </div>
                </motion.div>
              )}
            </AnimatePresence>
              ...
              */}
              ...
              >
                {widgets.map((widget) => {
                  ...
                  } else if (widget.type === "prompt-window") {
                    Icon = Square;
